...
export default function AddProduct() {
  ...
  useEffect(() => {
    USerService.getAllPrivatePosts().then(
      (response) => {
        formData.append('owner', response.data.username)
      },
      (error) => {
        console.log(error)
      }
  ...
  return (
    <div
      className="mt-24 
    mb-10 flex
    h-auto w-full flex-col items-center justify-center  border-2 border-y-green-400 bg-white shadow-xl shadow-slate-300 md:ml-10 md:w-[50%]"
    >
      ...
      <Formik
        ...
        onSubmit={(values) => {
          ...
          axios
            .post('http://localhost:128/regist', formData)
