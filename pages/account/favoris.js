import Annonce from '../../components/admin/annonce/Annonce'
import AnnonceDetails from '../../components/admin/annonce/AnnonceDetails'
import Favori from '../../components/admin/favori/Favori'
import FavoriDetails from '../../components/admin/favori/FavoriDetails'
import FavoriOptions from '../../components/admin/favori/FavoriOptions'
...
export default function favoris() {
  ...
  useEffect(() => {
    USerService.getMyFavoris().then((response) => {
      setWish(response.data.items)
    })
  }, [])
  return (
    <div>
                ...
                <tbody>
                  {wish.map(
                    (product, index) => (
                      ...
                      <Product
                        key={product.produit.id_pro}
                        details={product.produit.descpription}
                        prix={product.produit.prix}
                        photo={product.produit.image}
                      />
                      // </div>
                    )
                    //   )
